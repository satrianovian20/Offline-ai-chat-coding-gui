...
def start_llama(model_path):
    ...
    status_label.config(text="Memuat model...")
    ...
    if port_in_use(LLAMA_PORT):
        messagebox.showerror("Port Bentrok", f"Port {LLAMA_PORT} sudah digunakan oleh aplikasi lain.")
        return
    ...
    try:
        ...
        status_label.config(text="❌ Error saat memuat")
...
root = tk.Tk()
root.title("🦙 LLaMA.cpp GUI - Light & Low-End Ready")
...
status_label = ttk.Label(frame_top, text="🔄 Menunggu...", foreground="blue")
