...
class MultichatAI:
    ...
    def show_settings(self, tab_index):
        ...
        config = self.chat_tabs[tab_index].get("config", {})

        for f in fields:
            ttk.Label(win, text=f).pack()
            entry = ttk.Entry(win)
            entry.insert(0, str(config.get(f, "")))
        ...
        def simpan():
            for f in fields:
                ...
                if val:
                    try:
                        config[f] = float(val) if f in ["temperature", "repeat_penalty", "top_p"] else int(val)
                    except:
                        pass
            self.chat_tabs[tab_index]["config"] = config
            win.destroy()
