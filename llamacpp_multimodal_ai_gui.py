...
class MultichatAI:
    ...
    def query_server(self, full_prompt, display, use_chunk, delay, tab_index, stop_event, system_prompt, prompt, is_multimodal):
        try:
            ...
            # Deteksi multimodal base64 image
            if is_multimodal:
                print(f"Upload mode: {upload_mode}")
                if upload_mode == "embedding":
                    if 'latest_image_embedding' in globals():
                        payload["image_embedding"] = latest_image_embedding.squeeze(0).tolist()
                        print("[DEBUG] image_embedding dimasukkan ke payload (mode: embedding)")
                    else:
                        print("[DEBUG] latest_image_embedding tidak ditemukan (mode: embedding)")
                        display.insert(tk.END, "\n[⚠️ Gagal menemukan image embedding]\n")
                        display.see(tk.END)
                else:
