...
def stream_output(process):
    with open(LOG_FILE, "a", encoding="utf-8") as log, open(ERR_FILE, "a", encoding="utf-8") as errlog:
        ...
        for line in process.stderr:
            ...
            tab_chat.see(tk.END)
            if "ggml: format version" in decoded:
                status_label.config(text=f"📦 {decoded.strip()}")
...
def start_llama(model_path):
    ...
    if port_in_use(LLAMA_PORT):
        messagebox.showerror("Port Error", f"Port {LLAMA_PORT} sedang digunakan.")
        return
    ...
    try:
        ...
        if llama_process.poll() is not None:
            if ctx_size > 2048:
                tab_chat.insert(tk.END, "[⚠️] Gagal, mencoba ctx-size lebih kecil...\n")
                ctx_size = 2048
        ...
        status_label.config(text="✅ Model siap")
...
def send_message():
    ...
    system_prompt = system_input.get("1.0", tk.END).strip()
    ...
    try:
        ...
        with open(HISTORY_FILE, "a", encoding="utf-8") as h:
            h.write(f"You: {prompt}\nAI: {response}\n\n")
        history_box.insert(tk.END, f"You: {prompt[:50]}...\nAI: {response[:50]}...\n\n")
        auto_summary(response)
    except Exception as e:
...
def auto_summary(text):
    if len(text.split()) > 100:
        tab_chat.insert(tk.END, f"[📌 Summary]: {text[:200]}...\n")
...
progress = ttk.Progressbar(frame_main, variable=progress_var, maximum=100); progress.pack(fill="x", padx=10)
status_label = tk.Label(frame_main, text="⏳ Menunggu..."); status_label.pack()

tab_chat = scrolledtext.ScrolledText(frame_main, wrap=tk.WORD, height=20, width=120)
...
# System Tab
tk.Label(frame_system, text="System Prompt:").pack()
system_input = scrolledtext.ScrolledText(frame_system, height=5); system_input.pack(padx=10, pady=5)

history_box = scrolledtext.ScrolledText(frame_system, height=10); history_box.pack(padx=10, pady=5)
ttk.Button(frame_system, text="🗑️ Clear History", command=clear_history).pack()
