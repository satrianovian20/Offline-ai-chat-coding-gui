...
def stream_output(process):
    with open(LOG_FILE, "a", encoding="utf-8") as log, open(ERR_FILE, "a", encoding="utf-8") as errlog:
        ...
        for line in process.stderr:
            ...
            tab_chat.see(tk.END)
            if "model: loaded" in decoded.lower():
                version = decoded.strip()
                status_label.config(text=f"✅ Model berhasil dimuat: {version}")
            progress_bar.stop()

def start_llama(model_path):
    global llama_process, model_loaded, ctx_size
    model_loaded = False
    status_label.config(text="⏳ Memulai server model...")
    ...
    if port_in_use(LLAMA_PORT):
        messagebox.showerror("Error", f"Port {LLAMA_PORT} sudah digunakan oleh proses lain.")
        status_label.config(text="❌ Port bentrok.")
        progress_bar.stop()
        return
    ...
    try:
        ...
        if llama_process.poll() is not None:
            if ctx_size == 4096:
                ctx_size = 2048
                tab_chat.insert(tk.END, "[⚠️] Gagal ctx 4096, mencoba ulang dengan ctx 2048...\n")
        ...
        progress_bar.stop()
        status_label.config(text="❌ Error saat menjalankan model.")
...
def send_message():
    ...
    with open(HISTORY_FILE, "a", encoding="utf-8") as h: h.write(f"You: {prompt}\n")
    try:
        r = requests.post(LLAMA_API, json=data, timeout=120)
        r.raise_for_status()
        response = r.json().get("content","(no response)").strip()
        tab_chat.insert(tk.END, f"🤖 AI: {response}\n")
        with open(HISTORY_FILE, "a", encoding="utf-8") as h: h.write(f"AI: {response}\n")
    except Exception as e:
...
def clear_history():
    if messagebox.askyesno("Konfirmasi", "Hapus seluruh riwayat chat?"):
        try:
            open(HISTORY_FILE,"w").close()
            tab_chat.delete("1.0", tk.END)
            tab_chat.insert(tk.END, "💬 Riwayat chat dibersihkan.\n")
        except Exception as e:
...
notebook.add(frame_system, text="📋 Sistem")
...
ttk.Button(frame_top, text="🧹 Clear History", command=clear_history).pack(side="left", padx=5)
...
status_label = tk.Label(frame_main, text="Status: Belum ada model", fg="blue"); status_label.pack()
progress_bar = ttk.Progressbar(frame_main, mode="indeterminate"); progress_bar.pack(fill="x")
...
# Tab tambahan instruksi
system_label = tk.Label(frame_system, text="Instruksi sistem dan pengaturan tambahan akan ditambahkan di sini.")
