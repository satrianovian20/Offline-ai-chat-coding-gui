...
def start_rakyat_mode():
    path = filedialog.askopenfilename(title="Pilih Model GGUF (Rakyat Mode)", filetypes=[("GGUF files","*.gguf")])
    if path:
        tab_chat.insert(tk.END, "[🌾] Menjalankan dalam RAKYAT MODE: ctx-size 1024, RAM hemat!\n")
        threading.Thread(target=start_llama, args=(path, 1024), daemon=True).start()
...
ttk.Button(frame_top, text="🧑‍🌾 Rakyat Mode", command=lambda: threading.Thread(target=start_rakyat_mode, daemon=True).start()).pack(side="left", padx=5)
...
if "last_model_path" in conf:
    ...
    if os.path.isfile(LLAMA_EXE) and os.path.isfile(model_path):
        if conf.get("auto_load_rakyat_mode", False):
            ...
            threading.Thread(target=start_llama, args=(model_path, 1024), daemon=True).start()
        else:
